...
class MyModel(nn.Module):
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, train: bool,
        ...
        input_data = input_data.transpose(1, 2).contiguous()  # [64, 307, 12, 3]
        input_data = input_data.view(
            batch_size, num_nodes, -1).transpose(1, 2).unsqueeze(-1)  # [64, 36, 307, 1]
        time_series_emb = self.time_series_emb_layer(input_data)  # [64, 32, 307, 1]

        node_emb = []
        if self.if_spatial:
            # expand node embeddings
            node_emb.append(self.node_emb.unsqueeze(0).expand(
                batch_size, -1, -1).transpose(1, 2).unsqueeze(-1))  # [64, 32, 307, 1]
        # temporal embeddings
        tem_emb = []
        ...
        hidden = hidden.transpose(1, 2).unsqueeze(-1)  # [B, hidden_dim, N, 1]
